...
The [CMTO collector](cmto-run.md) now implements stable extraction, source-locator validation, and generator/evaluator wiring. Review the extracted context and acknowledge its exact hash before explicit execution. The arithmetic smoke command remains unchanged. CMTO runs freeze models, sources, prompts/rubric, shared candidate ordering and call/estimated-spend guards; they are not calibrated benchmarks or approval queues.
